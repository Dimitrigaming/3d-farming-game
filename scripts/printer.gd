...
func start_print() -> void:
	if is_printing or print_finished:
		return
	get_node_or_null("/root/GameLogger").info("Printer", "print started  duration=%.1fs" % print_duration)
	is_printing = true
...
func _on_print_complete() -> void:
	is_printing = false
	print_finished = true
	get_node_or_null("/root/GameLogger").info("Printer", "print complete  ready to collect")
	_update_tooltip()
