...
# Tiers expand depth (Z axis) — front door to ShopFloor is fixed, back wall is free.
# Depth goes 10 → 16 → 22 units.
const _DEPTHS: Array[float] = [10.0, 16.0, 22.0]
...
func apply_tier(tier: int) -> void:
	...
	# Side walls stretch in Z — they're rotated 90° so local size.x = world depth
